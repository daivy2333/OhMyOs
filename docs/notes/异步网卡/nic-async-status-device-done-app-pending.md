...
**日期**：2026-08-22（2026-08-29 更新至应用侧完成；2026-09-05 更新至故障恢复闭环）
**标签**：rust, async, network, nic, driver, socket, polling, syscall

> 完成 MS4（收包异步化）、MS5（有界双向数据面）、MS6（应用可见的异步网络）与 MS7（故障恢复语义）之后，网卡收发从设备侧到应用侧全链路异步化已经打通，设备出错或链路断开后也能安全恢复。本文说明这条链路的每一层怎么异步化、怎么做、最终卡点在哪。
...
全链路异步打通的是正常收发路径，没有回答"这条路断掉时怎么收尾"：设备复位后谁持有缓冲、请求被取消后各层怎么回收、包永远不回来时怎么判定超时、上个时代的完成包会不会落到新会话头上。MS7 在单 hart QEMU 下闭环了这些场景：设备 reset、分层取消、分阶段超时与链路断开/恢复都做了确定性处理，断在任意一步都不再产生悬垂引用、重复回收、永久挂起或静默丢包。

验收已收口：单 hart QEMU VirtIO-MMIO 上手工跑完整六个场景（reset 前流量、队列停滞→恢复、旧 socket 终止、新时代双向流量、HMP 关/开链路）按序全部通过，四组既有回归（异步收包、双向数据面、应用可见异步、设备基础）全过。过程中修掉两个产品 bug：重建队列读到陈旧 used-ring（`Dma::new` 分配 DMA 页未清零）导致 token 越界；`ppoll` 对零 `nfds` 的 NULL 指针返回地址错误。
...
先选单 hart QEMU，是因为恢复与并发竞态要在能注入故障的模型里先闭环，再放大到多核（下一步 SMP）和真板——QEMU 侧闭环后，重心转向 k3 板级冒烟与真板排查，避免直接上手时无法定位归属。
